...
@torch.no_grad()
def score_pair_rel(z: torch.Tensor, relW: torch.Tensor, a: torch.Tensor, b: torch.Tensor, r: torch.Tensor) -> torch.Tensor:
...
def main():
    ...
    ap.add_argument("--out", default="calib/temperature.json")
    args = ap.parse_args()
    ...
    print("⏳ Collect logits/labels for calibration...")
    for (a, b) in calib_pairs:
        ...
        a_t = torch.tensor([int(a)], device=device)
        b_t = torch.tensor([int(b)], device=device)

        r_pos = torch.tensor(pos, device=device, dtype=torch.long)
        a_rep = a_t.repeat(len(pos))
        b_rep = b_t.repeat(len(pos))
        lp = score_pair_rel(z, relW, a_rep, b_rep, r_pos).detach().cpu().numpy()
        logits_parts.append(lp)
        labels_parts.append(np.ones_like(lp))

        r_neg = torch.tensor(neg, device=device, dtype=torch.long)
        a_rep = a_t.repeat(len(neg))
        b_rep = b_t.repeat(len(neg))
        ln = score_pair_rel(z, relW, a_rep, b_rep, r_neg).detach().cpu().numpy()
        logits_parts.append(ln)
        labels_parts.append(np.zeros_like(ln))

    if not logits_parts:
        raise ValueError("No calibration samples collected. Check split files and dataset consistency.")

    logits = torch.tensor(np.concatenate(logits_parts), device=device, dtype=torch.float32)
    labels = torch.tensor(np.concatenate(labels_parts), device=device, dtype=torch.float32)
    ...
    def closure():
        opt.zero_grad()
        temperature = torch.exp(logT) + 1e-6
        loss = bce(logits / temperature, labels)
        loss.backward()
        return loss

    opt.step(closure)
    temperature = float((torch.exp(logT) + 1e-6).item())

    os.makedirs(os.path.dirname(args.out) or ".", exist_ok=True)
    with open(args.out, "w", encoding="utf-8") as f:
        json.dump({"T": temperature}, f, indent=2)

    print(f"✅ Fitted temperature: T={temperature:.6f}")
